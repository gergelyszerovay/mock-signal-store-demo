...
function asSpy<TArgs extends readonly any[] = any[], TReturnValue = any>(
...
function getCallCount<TArgs extends readonly any[] = any[], TReturnValue = any>(
  ...
  return asSpy(fn).callCount;
}

function getLastCallArgs<
  TArgs extends readonly any[] = any[],
  TReturnValue = any
>(fn: (...x: TArgs) => TReturnValue): TArgs {
  return asSpy(fn).lastCall.args;
}

function resetHistory<TArgs extends readonly any[] = any[], TReturnValue = any>(
  fn: (...x: TArgs) => TReturnValue
): void {
  asSpy(fn).resetHistory();
}

function getSpyFn<T extends any[]>(...args: T) {
  return fake<T>();
}
...
describe('ArticleListComponent_SS - mockComputedSignals: true + mock all child components', () => {
  ...
  beforeEach(async () => {
    ...
    // const store3 = fixture.debugElement.injector.get(ArticleListSignalStore);
    // const store3 = new ArticleListSignalStore();
    // patchState(store3, () => ({
    //   httpRequestState: HttpRequestStates.FETCHING,
    // }));

    // const store2 = asMockSignalStore(store, getSpyFn);
    // store.setPageSize
    // store2.setPageSize
